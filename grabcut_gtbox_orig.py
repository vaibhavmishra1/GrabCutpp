...
class outputs:
    ...
    def save_results(self, img, mask, logits, name):
        ...
        imageio.imwrite('/DATA/mishra.4/mishra.4/grabcut_orig_results/' + name + '.jpg', temp)
...
for index in range(length_names):
    ...
    cv2.grabCut(img, tmp, bbox_tupule, bgdModel, fgdModel, 5, cv2.GC_INIT_WITH_RECT)
    prediction_mask = np.where((tmp==2)|(tmp==0),0,1).astype('uint8')
    ...
    # print(np.amax(mask), np.amin(mask))
    # print(np.amax(prediction_mask), np.amin(prediction_mask))
    #heatmap_output = dismap
    ...
    # ret, heatmap_output = cv2.threshold(heatmap_output, 0, 255, cv2.THRESH_OTSU)
